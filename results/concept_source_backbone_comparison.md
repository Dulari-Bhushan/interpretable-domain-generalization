...
**Status: ✅ Done — result confirmed on CUB.** A later PACS extension (§8, using CLIP-distilled pseudo-labels to sidestep PACS's lack of real concept labels) failed clearly — see that section before assuming this result generalizes past CUB.
...
### PACS extension — DINOv2 via CLIP-distilled pseudo-labels: a clear failure to transfer

§6's own limitation ("this only tests CUB... any other dataset needs new per-image concept labels") prompted a follow-up question during review: since PACS has no real concept labels, could DINOv2's probe be trained against **CLIP's own zero-shot scores as soft pseudo-labels** instead — no real annotation needed anywhere? Built and run: CLIP zero-shot cosine similarity on PACS's photo train split (human-written 70-concept bank, min-max normalized per concept into a [0,1] soft BCE target — standard distillation, no arbitrary thresholding), DINOv2 ViT-L/14 probe trained against those targets, same downstream classifier protocol as PACS's CLIP-vs-LLM comparison above. Script: [`external/LanCE/experiments/pacs_dinov2_clip_distilled.py`](../external/LanCE/experiments/pacs_dinov2_clip_distilled.py); raw results: [`pacs_dinov2_clip_distilled.json`](pacs_dinov2_clip_distilled.json).

**Stated precisely before the numbers, since it matters for reading them correctly:** this design cannot show DINOv2 *exceeding* CLIP's concept-level judgment — CLIP's own zero-shot score *is* the training target, so at best DINOv2 learns to imitate it. The open question was narrower: does a DINOv2-backed classifier, trained only on CLIP's photo-domain calls, generalize better under shift than CLIP's own zero-shot judgment does on those same domains.

| Domain | CLIP zero-shot (human bank, no DDO) | DINOv2 + CLIP-distilled | Δ |
|---|---|---|---|
| photo (in-domain) | 99.70% | 99.10% | −0.60 |
| art_painting (shift) | 92.44% | 82.93% | −9.51 |
| cartoon (shift) | 94.46% | 59.28% | **−35.18** |
| sketch (shift) | 91.86% | 66.79% | −25.07 |

**No — it's substantially worse, not better, and the gap is far larger than PACS's own LLM-concept-bank result above.** The likely reason is visible directly in the training log, not just inferred from the accuracy drop: the probe's BCE loss barely moved over 50 epochs (0.689 → 0.636), and 0.693 (`ln 2`) is exactly the loss a model gets by predicting 0.5 for everything — the probe came away only marginally better than random guessing at reproducing CLIP's own soft scores. Compare CUB's real-label version of this same probe-training step (Plan 07 Stage 1): BCE loss 0.31 → 0.13-0.17, roughly half, real learning. Here, the distillation signal itself was too weak to teach DINOv2 much of anything resembling CLIP's actual concept judgments.

**Why in-domain accuracy still held up (99.10%) despite that weak signal, honestly reasoned through, not just observed:** DINOv2's raw features are strong enough that *any* fixed 1024→70 projection, even a poorly-fit one, likely retains enough incidental class-discriminative structure to support a strong downstream classifier for whatever domain that projection happened to be shaped by (photo) — but a projection shaped by noise rather than genuine, text-grounded concept semantics has no reason to carry that structure into a different visual domain, and evidently didn't. CLIP's own zero-shot scores are grounded directly in domain-invariant text ("a wagging tail" means the same thing regardless of visual style); a barely-converged distilled probe has no equivalent anchor, and the result — a domain-shift collapse far worse than anything else measured in this project on PACS — is consistent with that.
...
What *would* clear the 90% bar, if picked up: whether this DINOv2-concept-source approach can be **combined with domain-shift-specific machinery** (Component 2's grounded-DDO-style mechanism, adapted to a non-CLIP concept space) to add to, not just replace, its already-large domain-shift gain — genuinely unknown, since nothing in this project has tried a non-text-based domain-shift regularizer yet. That is new engineering (Component 2's mechanism was built assuming CLIP-text-embedding concept space), not a quick follow-up.

**Update: the CUB-only limitation (originally stated above) was tested, not just noted, and the one cheap way around it (CLIP-distilled pseudo-labels instead of real per-image labels, §8) failed clearly on PACS** — training loss barely moved past a random-guessing floor, and domain-shift accuracy collapsed by 9-35 points relative to CLIP zero-shot. This closes off the distillation route as a way to extend Plan 07's result past CUB without real annotation; it does not itself say anything new about whether real-labeled extensions (Office-Home, if concept-labeled; or genuine human annotation of PACS) would replicate CUB's finding — that remains a real, expensive, unanswered question, not a ruled-out one.
