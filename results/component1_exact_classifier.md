...
**What this result does not claim:** it doesn't establish that the L2-surrogate DDO term produces the same *trained model* as the original L1 version — only that, whichever version is used, the incremental update matches joint retraining of that same version exactly. It's validated on 4-domain, 7–65 class benchmarks, not yet at DomainNet's scale (6 domains, 345 classes).

## 12. What's next

Not a dead end — real, concrete follow-ups, ranked by priority:

1. **The L1-vs-L2 DDO ablation.** The one open question that could change how the L2 substitution is framed. Cheap — reuses existing cached embeddings and the existing SGD training script.
2. **Run it on DomainNet**, once a data loader exists for it (data is already downloaded — see `docs/new_methodology_report.md` §2.2). Tests whether the exact-match claim holds at a meaningfully larger scale (6 domains, 345 classes vs. 4 domains, 7–65 classes).
