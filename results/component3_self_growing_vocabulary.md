...
2. **Pruning** (`model/vocabulary_growth.py::prune_redundant_descriptors`): every pool entry embedded with a neutral placeholder noun, pairwise cosine similarity computed, later-added entries dropped if within 0.97 similarity of an earlier-kept one. Run on the grown pool regardless of whether growth added anything, so the mechanism is exercised end-to-end either way.
...
Best candidate (0.0522) reached roughly a third of the threshold and about a fifth of PACS's own comfort-zone alignment (Component 2's calibration point, ~0.25). Pool size: 204 → 204 (no change). Pruning consequently had 0 entries to drop.
...
Two real, different next steps, not a single "try harder" retry:

1. **Test a domain where CLIP's representation gap is smaller but the pool's coverage gap is still real** — the two things Phase F3 separated (representation-level alignment vs. pool-content coverage) are conflated in this one Midjourney v6 test, because both happen to be bad simultaneously for this domain. EuroSAT (Component 2's second calibration point, alignment 0.322–0.324 — weak but not near-zero like Midjourney's 0.037–0.05) is a better test of whether growth helps *when there's more room for wording to matter* — if candidates still fail to clear threshold there, that's stronger evidence the mechanism itself needs rethinking rather than just the domain choice.
2. **If growth keeps failing to find anything even on an easier domain, the honest conclusion is that growth-by-templated-generation is a dead end for domains this far outside CLIP's training distribution**, and Component 3's premise needs revisiting — e.g., growing the pool from real probe images directly (an image-derived phrase or embedding, sidestepping the text-generation step entirely) rather than assuming a better string exists to be found. That would blur the line between Component 2 (image-grounded diffs) and Component 3 (vocabulary growth), which is itself worth flagging honestly rather than treating as a new idea, if it's tried.
3. **Pruning is untested in any regime where it actually has something to drop.** This run's 0-candidates-accepted outcome means pruning ran but never exercised its core logic (dropping a near-duplicate). A synthetic check — seed the pool with a deliberately redundant phrase and confirm it gets dropped — would at least confirm the mechanism works in isolation, independent of whether growth ever produces something worth pruning in practice.
