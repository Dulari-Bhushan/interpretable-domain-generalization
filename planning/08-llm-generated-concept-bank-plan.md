...
**Done, replicated, and extended to PACS — results in [`results/llm_concept_bank_comparison.md`](../results/llm_concept_bank_comparison.md). Mixed result: the effect reverses between datasets.** On CUB, the LLM-generated bank won on every metric (in-domain and CUB-Painting shift, with and without DDO) on two independent draws — draw 1 by 1.6-3.2 points, draw 2 by 2.0-4.7 points. On PACS, the same approach **lost** on every metric, up to -11.7 points on the hardest domain (sketch) with DDO. Read the full writeup before citing either direction alone.
