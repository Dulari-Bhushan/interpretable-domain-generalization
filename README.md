...
**Every component gets a full write-up, once it's actually been run, following one fixed convention** — see [`docs/component_report_template.md`](docs/component_report_template.md) for how these are structured (method, which failure hypothesis it targets, dataset choices, real results, honest interpretation, and either a concrete next step or a plainly-stated dead end). The first one, filled out for real: [`results/component1_exact_classifier.md`](results/component1_exact_classifier.md).

Status so far:
- ✅ **Component 1 — exact, no-forgetting classifier update: fully done, including the scale test.** Validated on PACS, Office-Home, and now **DomainNet (6 domains, 345 classes, ~586K images)**. Result: **max difference from the joint/oracle fit = 0.0000** on every domain ordering tested, on all three datasets — including Office-Home, where the original SGD-trained baseline showed real forgetting (BWT −0.68 to −4.68) that standard remediations only partially fixed, and DomainNet, which pushed the claim to real scale and found no degradation at all. Full report: [`results/component1_exact_classifier.md`](results/component1_exact_classifier.md); raw numbers in `results/component1_pacs_results.json`, `results/component1_officehome_results.json`, `results/component1_domainnet_results.json`.
  - **1b — L1-vs-L2 DDO ablation:** done, mixed result. The substitution needed for the exact update is free on PACS, but costs 1.75 accuracy points on Office-Home in exchange for a 5–26x stronger orthogonality property. Full report: [`results/component1b_l1_vs_l2_ablation.md`](results/component1b_l1_vs_l2_ablation.md).
...
- ⚠️ **How well can a VLM generate concepts? Done, mixed result.** Compared Plan 08's text-only LLM concept bank against image-grounded VLM prompting (Gemini, Qwen2-VL-7B) and a non-generative Grounding-DINO + CLIP retrieval pipeline. Neither image-grounded VLM beat text-only generation on concept quality or downstream accuracy once concept-bank size was controlled for (a real confound: most of their apparent full-size advantage was just having far more concepts, not better ones) — the one pipeline with a genuine edge over the weaker baselines isn't a VLM at all. Full report: [`results/vlm_concept_generation_comparison.md`](results/vlm_concept_generation_comparison.md).
...
| **AWA2** | Component 1 (planned, not yet run) | 37,322 imgs, 50 classes | 2017 (v2) | A loader already existed in the vendored codebase from before this project (unused); downloaded now that dataset scale isn't a constraint — a third, ~free confirmation of Component 1 beyond PACS/Office-Home |
| **DomainNet** | Component 1 — **done** | ~0.6M imgs, 6 domains, 345 classes | 2019 | Downloaded, loader built (`external/LanCE/data/DomainNet/`), and Component 1's exact-match claim confirmed at this scale — see `results/component1_exact_classifier.md` §9 |
...
- `experiments/component1_analytic_domain_il.py`, `experiments/component1_analytic_domain_il_officehome.py`, `experiments/component1_analytic_domain_il_domainnet.py` — validation harnesses reusing `DomainILSession`'s existing cached embeddings; compare the analytic classifier's sequential fit against a joint/oracle fit on the same data, per domain ordering.
- `experiments/component1_l1_vs_l2_ablation.py` — trains the original SGD+L1-DDO joint model alongside the analytic L2-DDO classifier on identical data, comparing accuracy and the actual orthogonality property DDO optimizes for.
- `data/DomainNet/` — new loader built for Component 1's scale test: `prepare_domainnet_dataset.py`, `domainnet_data.py`, `generate_domainnet_concepts.py` (a template-generated concept bank, deliberately not hand-written — see the script's own docstring for why).
...
- ~~A DomainNet data loader~~ — **done**, and Component 1's scale test on it is done too (see `results/component1_exact_classifier.md`). Open follow-up: the DomainNet concept bank is template-generated, not hand-written (deliberate, stated in `external/LanCE/data/DomainNet/generate_domainnet_concepts.py`) — would need real concepts if a future experiment needs DomainNet classification *accuracy* to mean something, not just Component 1's exactness property.
