...
| Path | What's there |
|---|---|
| [`docs/`](docs) | The failure-mode analysis of LanCE (five failure modes, each traced to a specific equation/table in the paper) — **now updated throughout with every phase's actual measured result**, not just the original predictions |
...
**Both pillars are done, and Phase G (folding every measured result into `docs/lance_continual_dg_failure_analysis.md` and the presentation artifact) is done too** — both files have been updated throughout to replace every original prediction with what was actually measured, including the null/surprising results. See "Further plans" below for what's genuinely still open.
...
- **A complete GenImage download.** Phase F4 only recovered 155/1,000 ImageNet classes and no real labeled training photos (partial multi-part archive — see `results/phase_f4_genimage_alignment.md`). A full download would enable the test closest to the original plan: an actual baseline-vs-+DDO training run on a genuinely novel domain (not just an alignment score).
- **Descriptor-set staleness test** (originally planned item 4, never run): freeze the descriptor pool at one LLM's knowledge cutoff, test against a domain only a *newer* LLM would name. Distinct from what Phase F1/F3/F4 tested (those measure CLIP's image-text alignment, not the descriptor-generating LLM's knowledge boundary specifically).
