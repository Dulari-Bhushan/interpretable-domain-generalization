...
## 5. The combined/ablation table — built (2026-08-19), one real experiment still open

The project owner's original ask (restated for history): since each component fixes a *different* problem, test them **combined**, not just individually — a real ablation table against the original baseline and each component alone.

**Done**: `docs/new_methodology_report.md` §9 now has the real table, every cell either a genuine already-measured number (Component 1 alone: PACS/Office-Home/DomainNet exact-match; Component 2 alone: Defactify blend, +1.17 mean gain across seeds/domains; Component 3 alone: Defactify no-op) or explicitly marked **not run**, with the precise reason: Component 1 was validated on a domain-IL *sequence*, Components 2/3 on a *single new-domain arrival* — different experimental shapes, nothing to combine into a cell without a new run.

**The one well-defined experiment that would fill the combined rows**, spelled out concretely in §9 rather than left vague: build a 6-domain sequence from Defactify itself (real photos + all 5 generators, sharing one class set) — several of those arriving domains are genuinely low-alignment, exactly Component 2's use case, unlike PACS/Office-Home which were deliberately all CLIP-comfort-zone. This needs two real pieces of new engineering, not just a rerun: (1) a Defactify-as-domain-sequence loader (doesn't exist), and (2) `AnalyticDomainIncrementalClassifier` (`external/LanCE/model/analytic_classifier.py`) currently bakes its DDO regularizer term into `R` once at construction from one fixed `domain_diffs` — it has no way to add a *different* diagnosed term per arriving domain, which Component 2's mechanism needs. Checked mathematically (not just asserted) that extending `partial_fit` to add each domain's own term still preserves the exact-match-to-joint property, since `R` only ever accumulates by matrix addition regardless of order — the code to do this doesn't exist yet.

**Not started, and deliberately not started without asking first** — this is a real scope commitment (new loader + a change to Component 1's own core class + a new validation script), not a quick follow-up. Component 3 was checked and correctly excluded from being useful in a first combined run: it's currently a verified no-op on the only domain tested (0 candidates ever cleared the trust threshold), so "Component 1 + Component 3" wouldn't currently be distinguishable from "Component 1 alone."
