...
Component 2 tracker status in `docs/new_methodology_report.md` reflects this: "diagnostic validated; fallback mechanism identified, not yet statistically confirmed" — not "validated."
...
**Still open, not started (deliberately - not run without checking first, given the size of the claims resting on it):** a seed sweep (3-5 seeds, cheap - frozen CLIP features, linear-layer training) on the 5 key conditions, which is what would actually license the magnitude claim above. Also open: a second validation domain (everything so far is one domain shift, photo→Midjourney v6). See `results/component2b_grounding_fallback_variants.md` "What's next" #1 and #3. **The combined/ablation-testing commitment (§5 below) is explicitly NOT triggered by this** - don't treat Component 2 as a second validated component yet.
