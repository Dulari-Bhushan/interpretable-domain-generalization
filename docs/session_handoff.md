...
**Main experiment: done** (ran 2026-08-19 ~19:58-20:15 IST on **GPU 1** of lab-server, tmux session `c2_defactify`, now exited cleanly). **Result: partial/mixed, written up honestly at [`results/component2_self_diagnosing_grounding.md`](component2_self_diagnosing_grounding.md).** The diagnostic correctly flagged photo→Midjourney v6 as untrustworthy (probe alignment 0.0123, threshold 0.1431) - that part works. But the fallback it triggered (a single image-measured `domain_diffs` direction from the 20/class probe) made target accuracy *worse* than baseline (73.81% vs. baseline's 74.27%; text-only DDO reached 75.03%, closely reproducing Phase E2's own +0.68 gain with a +0.76 here). Read the report's "What's next" section before resuming this - it lists 4 concrete follow-ups (preserve directional diversity in the fallback instead of one mean direction; probe-size sweep; blend instead of replace; EuroSAT as a second domain), ranked most-promising first.
