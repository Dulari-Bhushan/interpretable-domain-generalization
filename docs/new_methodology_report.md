...
**Status: not started as an experiment.** This section is the "it's now a real, written part of the plan" step the commitment asked for — the run itself still needs Component 3 (and ideally Component 4) to reach at least a first result before a combined condition is meaningful. With Component 1 done and Component 2's diagnostic + blend fallback now validated across 3 seeds and 2 domains, there is already enough validated to define what the table looks like:
