...
**Status: Component 1 done and validated. Component 2's diagnostic + blend fallback validated across 3 seeds x 2 domains; persample fallback promising, not yet fully confirmed. Component 3 done — mechanism validated as safe, not (yet) effective. Components 4–5 not started. Report updated as work continues.**
...
| 2 | Self-diagnosing domain grounding | DDO predicts a new domain's look purely from text, with measured near-zero reliability for domains that matter (Phase F1/F3/F4) | ✅ **Diagnostic + blend fallback validated across 3 seeds × 2 domains; persample shows a promising edge, not yet fully confirmed** — see `results/component2c_seed_sweep_and_second_domain.md` |
...
4. **Start Component 2** — ✅ done, see [`results/component2_self_diagnosing_grounding.md`](../results/component2_self_diagnosing_grounding.md), [`results/component2b_grounding_fallback_variants.md`](../results/component2b_grounding_fallback_variants.md), and [`results/component2c_seed_sweep_and_second_domain.md`](../results/component2c_seed_sweep_and_second_domain.md) (diagnostic + blend fallback validated across 3 seeds and 2 domain shifts; persample shows a promising, not-yet-fully-confirmed edge over text-only DDO).
...
**Status: single-component rows filled from already-run results below; every genuinely combined row is honestly marked not yet run, with the specific reason and what it would take.** Per this project's one standing rule (`docs/component_report_template.md`): no cell below reports a number that wasn't actually produced by a real run.

| Condition | Benchmark | Result | Status |
|---|---|---|---|
| Original SGD baseline (no components) | Office-Home, hardest ordering | BWT −4.68 | Measured (Phase D) |
| Original SGD baseline (no components) | Defactify, photo→Midjourney v6 | 74.27–74.57% target acc (α=0) | Measured (Component 2/3 runs) |
| Component 1 alone | PACS / Office-Home / DomainNet, domain-IL sequence | max\|diff from joint\| = 0.0000 on every ordering tested | ✅ [`results/component1_exact_classifier.md`](../results/component1_exact_classifier.md) |
| Component 2 alone (blend) | Defactify, photo→Midjourney v6 + DALL-E 3 | mean gain +1.17 over baseline (matches text-DDO, tight std 0.14, 4 measurements) | ✅ [`results/component2c_seed_sweep_and_second_domain.md`](../results/component2c_seed_sweep_and_second_domain.md) |
| Component 3 alone | Defactify, photo→Midjourney v6 | numerically identical to +DDO-text (0 candidates accepted — correct no-op) | ✅ [`results/component3_self_growing_vocabulary.md`](../results/component3_self_growing_vocabulary.md) |
| Component 1 + Component 2 | — | — | **Not run** — see below |
| Component 1 + Component 3 | — | — | **Not run** — see below |
| All validated components combined | — | — | **Not run** — see below |

**Why the combined rows are empty, precisely, not just "not gotten to it":** Component 1 was validated on a *domain-IL sequence* (PACS/Office-Home/DomainNet — several domains arriving one after another, measured by BWT/exact-match-to-joint). Components 2 and 3 were validated on a *single new-domain arrival* (Defactify — one baseline-vs-+DDO comparison, no sequence). These are different experimental shapes, not just different datasets — there's no existing run that's simultaneously "a domain-IL sequence" and "a single arrival with a self-diagnosed grounding decision," so there's nothing to average or compare into a combined cell without a new run.

**What that new run would actually require, precisely (checked, not hand-waved):**
1. **A domain sequence where Component 2's grounding decision matters.** PACS/Office-Home's domains are all CLIP-comfort-zone by design (Component 1 needed that to isolate forgetting from alignment). Defactify already has the needed structure sitting unused for this purpose: treating real photos + all 5 generators (SD2.1, SDXL, SD3, DALL-E 3, Midjourney v6) as **six sequential domains sharing one class set** would give a domain-IL sequence where several arriving domains are genuinely low-alignment — exactly the case Component 2 exists for. Not built yet; needs a loader analogous to `data/PACS/pacs_data.py` and a check of which classes have enough samples across all 6 sources at once (likely fewer than the 23 used in Components 2/3's pairwise comparisons).
2. **`AnalyticDomainIncrementalClassifier` doesn't currently support a domain-varying DDO term.** Reading its own code (`external/LanCE/model/analytic_classifier.py:60-78`): the DDO regularizer is added to `R` **once, at construction**, from one fixed `domain_diffs` tensor — the original method's assumption (one static 204-descriptor pool) is baked into the `__init__` signature, not `partial_fit`. Component 2's mechanism needs a *different* `domain_diffs` diagnosed per arriving domain. This is a real code change, not a config flag — `partial_fit` would need to accept and add each domain's own diagnosed DDO term to `R` at that domain's arrival. Checked mathematically, not just assumed: this still preserves exactness, because `R` accumulates by simple matrix addition regardless of what's added when or in what order — the "joint" comparison baseline would just need redefining to also sum each domain's own grounded term, rather than one shared term. The math supports building this; it hasn't been built.
3. **Component 3 composes into the same combined row for free, since it was validated as an identical no-op on this exact domain** (grown pool == original pool) — "Component 1 + Component 3" on Defactify would currently just equal "Component 1 + Component 2 with grounding forced off," not a distinct test worth a separate run, until Component 3 has a domain where it isn't a no-op.

This is now the single most well-defined "next experiment" in the project — see the answer to "is there a meaningful experiment left" for the honest read on whether to build it.
