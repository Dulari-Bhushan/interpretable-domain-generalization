"""Component 2 - self-diagnosing domain grounding, validated on Defactify
(photo -> Midjourney v6, the exact domain shift Phase F3 and Phase E2
already measured).

Reruns Phase E2's baseline-vs-+DDO protocol (same data, same tagging
heuristic, same 50-epoch/batch-64/AdamW/lr=1e-4 setup) with one addition: a
third condition, +DDO with domain_diffs built by this project's own
self-diagnosing pipeline (model/domain_grounding.py) instead of the fixed
204-entry text descriptor pool.

Because the diagnostic needs a probe of real target-domain images that
Phase E2 never set aside, this script draws its own probe (PROBE_PER_CLASS
images/class) from the Midjourney pool, held out from target_test, and
reruns all three conditions (baseline, +DDO-text, +DDO-grounded) under one
consistent split rather than reusing Phase E2's saved numbers against a
different target_test set - keeps the 3-way comparison apples-to-apples.

Expected outcome per docs/new_methodology_report.md/Phase F3: the
diagnostic should report a low alignment score (near Phase F3's 0.037) and
correctly decide NOT to trust the text-only domain_diffs, falling back to
the image-grounded version - the interesting empirical question this
script answers is whether that grounded version recovers any of the DDO
gain Phase E2 found missing (+0.68 vs. +6.40 points).
"""
import os
import sys
import json
...
def build_entries(idx_by_label_cat, classes, class_to_id, rng):
    """Returns train/source_test/target_probe/target_test entries (each a
    list of (split, row_idx, class_id)) and a per-class count summary.
    Target Midjourney pool is split probe-first (held out from target_test)
    so the diagnostic never sees the images it's later evaluated against."""
...
def load_images_by_class(ds_dict, entries, class_names, id_to_class, preprocess):
    """entries: list of (split, row_idx, class_id). Returns dict[class_name]
    -> list of preprocessed image tensors, loaded directly (not cached -
    this is a small probe, tens of images/class, not worth the caching
    machinery cache_utils.py exists for)."""
...
def main():
    ...
    # Threshold from component2_alignment_calibration.py's PACS run, if
    # available; otherwise fall back to the midpoint computable from this
    # project's own already-published numbers (PACS not yet measured this
    # exact way = conservative fallback, documented honestly in the report
    ...
    print(f"\nRunning self-diagnosis on a {PROBE_PER_CLASS}/class probe "
          f"({SOURCE_TEMPLATE!r} -> {TARGET_TEMPLATE!r})...")
    # Source probe drawn fresh from the real-photo train pool (own RNG
    # stream, not overlapping target_probe/target_test which are the only
    # sets that matter for leakage - the source side is never held-out from
    # training in this project's convention, same as the text-only diffs
    # also never treating "photo" as a held-out concept).
    ...
    print(f"grounded_domain_diffs shape: {tuple(grounded_domain_diffs.shape)}")

    cache_prefix = f"Defactify_MJv6_C2_{args.CLIP_type}".replace("/", "-")
    ...
    ddo_text_gain = results_by_condition["ddo_text"]["best_target_acc"] - results_by_condition["baseline"]["best_target_acc"]
    ddo_grounded_gain = results_by_condition["ddo_grounded"]["best_target_acc"] - results_by_condition["baseline"]["best_target_acc"]
    print(f"\n=== Summary ===")
    print(f"Baseline target acc:      {results_by_condition['baseline']['best_target_acc']:.2f}%")
    print(f"+DDO (text) target acc:   {results_by_condition['ddo_text']['best_target_acc']:.2f}% (gain {ddo_text_gain:+.2f})")
    print(f"+DDO (grounded) target acc: {results_by_condition['ddo_grounded']['best_target_acc']:.2f}% (gain {ddo_grounded_gain:+.2f})")
    ...
    results = {
        ...
        "ddo_text_gain_points": ddo_text_gain,
        "ddo_grounded_gain_points": ddo_grounded_gain,
        "phase_e2_ddo_gain_points_reference": 0.68,
        "phase0_ddo_gain_points_reference": 6.40,
    }
    os.makedirs(RESULTS_DIR, exist_ok=True)
    out_path = os.path.join(RESULTS_DIR, "component2_defactify_grounding_ddo.json")
